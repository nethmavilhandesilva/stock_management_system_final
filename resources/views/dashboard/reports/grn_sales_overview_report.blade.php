...
<h2 class="company-name">{{ $companyName ?? 'Default Company' }}</h2>
    ...
    @php
        ...
        $soldPacksFromSales = 0;
        $soldWeightFromSales = 0;
        $grandTotalSalesValue = 0;
        $grandTotalRemainingPacks = 0;
        $grandTotalRemainingWeight = 0;
        $grandTotalPrice = 0; // New for price column
    @endphp
    ...
    @forelse(collect($reportData)->groupBy('grn_code') as $grnCode => $items)
        @foreach($items->groupBy('item_name')->sortKeys() as $itemName => $itemRecords)
            @php
                ...
                $subTotalSoldPacks = $itemRecords->sum('real_sold_packs');
                $subTotalSoldWeight = $itemRecords->sum('real_sold_weight');
                ...
                $subTotalPrice = $itemRecords->avg('price'); // Use avg or sum as needed
                 $subTotalPrice = $itemRecords->avg('sp'); // Or sum if needed

                $grandTotalOriginalPacks += $subTotalOriginalPacks;
                $grandTotalOriginalWeight += $subTotalOriginalWeight;
                $soldPacksFromSales += $subTotalSoldPacks;
                $soldWeightFromSales += $subTotalSoldWeight;
    ...
    <tr class="total-row">
        ...
        <td><strong>{{ number_format($soldWeightFromSales, 2) }}</strong></td>
        <td><strong>{{ number_format($soldPacksFromSales) }}</strong></td>
